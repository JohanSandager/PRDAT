...
type expr2 =
    | CstI1 of int
    | Var1 of string
    | Let1 of (string * expr2) list * expr2
    | Prim1 of string * expr2 * expr2
...
let rec eval2 e (env: (string * int) list) : int =
    ...
    | Var1 x -> lookup env x
    | Let1(lst, ebody) ->
        let env1 = List.fold (fun acc (v, e) -> (v, eval2 e env) :: acc) env lst
        eval2 ebody env1
    | Prim1("+", e1, e2) -> eval2 e1 env + eval2 e2 env
    | Prim1("*", e1, e2) -> eval2 e1 env * eval2 e2 env
    | Prim1("-", e1, e2) -> eval2 e1 env - eval2 e2 env
    | Prim1 _ -> failwith "unknown primitive"

// 2.2
let rec freevars1 e : string list =
    match e with
    | CstI1 i -> []
    | Var1 x -> [ x ]
    | Let1(lst, ebody) ->
        let result: string list =
            List.fold (fun acc (v, erhs) -> union (freevars1 erhs, acc)) [] lst

        let vars = List.fold (fun acc (v, _) -> v :: acc) [] lst
